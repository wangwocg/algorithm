...
extern int SWContentFilter( const char * buffer , int bufferlen, char * policynames , int nIfGetMime , jobject *obj , JNIEnv * penv ,SW_EVENT_INFO *sweventinfo,int * pIfFound );

extern int SWFileFilter( const char * path , char * policynames , int nIfGetMime , jobject *obj , JNIEnv * penv , SW_EVENT_INFO *sweventinfo,int * pIfFound );

extern int SWFilterInit();

extern int threadFilterInit( JNIEnv** penv , jobject * pobj );
extern void threadFilterClean();
...
int parser( char * filename ) {
		...
		printf("length=%d\n",length);
        ret = SWContentFilter( buffer , length , policynames ,1,NULL,NULL, &sweventinfo,&found );
		free(buffer);
		return 0 ;
}
void * runpath( void * p ){
	...
	JNIEnv * env = NULL ;
	jobject  *obj;
	threadFilterInit( &env , &obj );

	ret = SWFileFilter( p , policynames ,1,obj,env, &sweventinfo, &found );
